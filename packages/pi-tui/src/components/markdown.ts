	...
	private renderList(
		...
		for (let i = 0; i < token.items.length; i++) {
			...
			if (itemLines.length > 0) {
				...
				// A nested list will start with indent (spaces) followed by any SGR color + bullet
				const firstLine = itemLines[0];
				const isNestedList = /^\s+\x1b\[[\d;]*m[-\d]/.test(firstLine); // starts with spaces + SGR + bullet char
				...
				for (let j = 1; j < itemLines.length; j++) {
					...
					const isNestedListLine = /^\s+\x1b\[[\d;]*m[-\d]/.test(line); // starts with spaces + SGR + bullet char
